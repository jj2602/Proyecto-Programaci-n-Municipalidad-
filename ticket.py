...
def agregar_multa(ventana_padre, usuario_actual):
    ...
    def guardar_multa():
        ...
        # --- Ventana emergente del ticket ---
        ventana_ticket = Toplevel(multa)
        ventana_ticket.title(f"Ticket de {patente}")
        ventana_ticket.geometry("500x700")
        ventana_ticket.config(bg="white")

        Label(ventana_ticket, text="TICKET DE MULTA", font=("Arial", 16, "bold"), bg="white",fg="red").pack(pady=10)

        # Mostrar el texto del ticket
        txt_ticket = Text(ventana_ticket, wrap="word", font=("Courier New", 10))
        txt_ticket.pack(expand=True, fill="both", padx=10, pady=10)
        txt_ticket.insert("1.0", contenido_ticket)
        txt_ticket.config(state="disabled")

        # Mostrar imagen si existe
        if foto != "Sin foto" and os.path.exists(foto):
            try:
                img = Image.open(foto)
                img.thumbnail((300, 300))  # Redimensionar para que entre bien
                img_tk = ImageTk.PhotoImage(img)

                Label(ventana_ticket, text="Foto asociada:", bg="white", font=("Arial", 12, "bold")).pack(pady=5)
                lbl_img = Label(ventana_ticket, image=img_tk, bg="white")
                lbl_img.image = img_tk  # 🔥 Mantener referencia
                lbl_img.pack(pady=10)
            except Exception as e:
                messagebox.showwarning("Advertencia", f"No se pudo mostrar la imagen:\n{e}")

        Button(ventana_ticket, text="Cerrar", bg="#006172", fg="white", command=ventana_ticket.destroy).pack(pady=10)
...
def mostrar_multas(ventana_padre, patente, modo_usuario=False):
    ...
    if modo_usuario:
        ...
        def pagar_multa():
            ...
            qr = qrcode.QRCode(version=None, box_size=5, border=4)
