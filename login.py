...
class Login:
    def __init__(self):
        ...
        self.logo_img = Image.open("Imagenes/Muni.png").resize((200, 200))
    ...
    def login(self):
        ...
        if self.validar_usuario(usuario, contrasena):
            ...
            if usuario == "admin" and contrasena == "admin":
                AdminPanel()  # Abrir panel de admin
        else:
...
class Registro:
    ...
    def registrar_usuario(self):
        ...
        repetir_conntrasena = self.entry_repetir_contrasena.get()
        if not nuevo_usuario or not nueva_contrasena or not repetir_conntrasena:
            messagebox.showwarning("Registro", "Debe completar todos los campos.")
            return
        if nueva_contrasena != repetir_conntrasena:
...
class AdminPanel:
    def __init__(self):
        ...
        Label(self.ventana, text="Panel de Administración", font=("Arial", 24, "bold")).pack(pady=20)

        Button(self.ventana, text="Agregar Inspector", font=("Arial", 16), width=20, command=self.agregar_inspector).pack(pady=10)
        Button(self.ventana, text="Agregar Usuario", font=("Arial", 16), width=20, command=self.agregar_usuario).pack(pady=10)
        Button(self.ventana, text="Agregar Multa", font=("Arial", 16), width=20, command=self.agregar_multa).pack(pady=10)
        Button(self.ventana, text="Quitar Multa", font=("Arial", 16), width=20, command=self.quitar_multa).pack(pady=10)
